...
public class PrivoAgeGate {
    ...
    /// The method allows checking the existing Age Gate status.
    ///
    /// > Concurrency Note: You can call this method from asynchronous code. This method has a similar ``getStatus(userIdentifier:nickname:completionHandler:errorHandler:)``  one, which resembles the completion handler method for calling from synchronous code.
    /// >
    /// > See more information [about concurrency and asynchronous code in Swift](https://developer.apple.com/news/?id=o140tv24) .
    ///
    /// - Parameters:
    ///   - userIdentifier: external user identifier (please don't use empty string ("") as a value. It will cause an error. We support real values or nil if you don't have it)
    ///   - nickname: optional parameter with default value nil. Please, use nickname only in case of multi-user integration. Please don't use empty string "" in it.
    /// - Throws: there is only one exception type - PrivoError.
    ...
    /// - Parameters:
    ///   - data
    ///   - completionHandler: A closure to execute. Nil indicates that a failure occurred.
    ...
    /// The method runs the Age Gate check.
    ///
    /// > Concurrency Note: You can call this method from asynchronous code. This method has a similar ``run(_:completionHandler:)``  one, which resembles the completion handler method for calling from synchronous code.
    /// >
    /// > See more information [about concurrency and asynchronous code in Swift](https://developer.apple.com/news/?id=o140tv24) .
    ///
    /// If the birth date is passed by a partner or filled in by a user, the method will return the status. If the birth date is not passed, a user will be navigated to the corresponding entry window and forced to fill in the birthday field.
    ...
    /// The method allows rechecking data if the birth date provided by a user was updated.
    ///
    /// > Concurrency Note: You can call this method from asynchronous code. This method has a similar ``recheck(_:completionHandler:)``  one, which resembles the completion handler method for calling from synchronous code.
    /// >
    /// > See more information [about concurrency and asynchronous code in Swift](https://developer.apple.com/news/?id=o140tv24) .
    ///
    /// - Parameter data:
    ...
    /// The method links a user to specified userIdentifier.
    ///
    /// > Concurrency Note: For calling method from asynchronous code you could use a similar ``linkUser(userIdentifier:agId:nickname:)``.
    /// >
    /// > See more information [about concurrency and asynchronous code in Swift](https://developer.apple.com/news/?id=o140tv24) .
    ///
    /// It is used in multi-user flow in case an account creation (on a partner side) happens after the Age Gate flow.
    /// Please note that linkUser can be used only for users that don't have userIdentifier yet. It is impossible to change the userIdentifier after it was obtained.
    ///
    /// - Parameters:
    ...
    /// The method links a user to specified userIdentifier.
    ///
    /// > Concurrency Note: You can call this method from asynchronous code. This method has a similar ``linkUser(userIdentifier:agId:nickname:completionHandler:errorHandler:)``  one, which resembles the completion handler method for calling from synchronous code.
    /// >
    /// > See more information [about concurrency and asynchronous code in Swift](https://developer.apple.com/news/?id=o140tv24) .
    ///
    /// It is used in multi-user flow in case an account creation (on a partner side) happens after the Age Gate flow.
    ...
    ///     - ``PrivoError.cancelled`` is thrown if cancelled with `Task.cancel()`.
    ///     - ``PrivoError.incorrectInputData(AgeGateError)`` is thrown if the data submitted for processing is not correct.
    /// - Returns: AgeGateEvent instance, which takes into consideration a user age and configuration parameters defined by a partner.
    public func linkUser(userIdentifier: String,
                         agId: String,
